const zh = {
    ...
    addBrevoTokenDesc: '输入内容添加 Brevo API Key，不填则删除',
    brevoTokenList: 'Brevo Token 列表',
    brevoSender: 'Brevo 发件',
    brevoSenderDesc: 'Brevo 通过环境变量 BREVO_API_KEY 全局配置；启用后可在域名服务商映射中选择 brevo',
    brevoWebhookSecret: 'Brevo Webhook 密钥',
    brevoWebhookSecretPlaceholder: '输入密钥，留空则停用 Webhook',
    brevoWebhookSecretDesc: '首次升级时使用环境变量 BREVO_WEBHOOK_SECRET 初始化；保存后以此处配置为准。',
    domainProvider: '域名发件服务商',
    domainProviderDesc: '为指定域名强制使用某个发件服务商，优先级最高；不设置则按全局默认链（CF → Resend → Brevo）',
    provider: '服务商',
    setProvider: '设置域名服务商',
    providerOverrideTitle: '域名服务商映射',
    useDefault: '使用默认',
    ...
    dropFilesHere: '释放文件以上传',
    fileTooLarge64: '已跳过 {count} 个文件，单个文件必须大于 0 且不能超过 64 MB',
    attachmentCountLimit: '已跳过 {count} 个文件，每封邮件最多添加 10 个普通附件',
    fileUploadFailedTitle: '文件上传失败',
    fileUploadFailed: '{name} 上传失败：{message}',
    uploadingFiles: '文件正在上传，请稍候',
    ...
    storageType: '存储类型',
    migrateKvAttachments: '迁移 KV 附件',
    migrateKvAttachmentsDesc: '将 KV 中 attachments/ 开头的文件分批复制到 R2，保留 key、文件名和文件类型，不会删除 KV 数据',
    migrateKvAttachmentsConfirm: '确认开始将 KV 附件复制到 R2 吗？该操作可重复执行，且不会删除 KV 原数据。',
    startMigration: '开始迁移',
    migratingAttachments: '已迁移 {count}',
    migrateKvAttachmentsSuccess: '迁移完成：成功 {migrated} 个，跳过 {skipped} 个',
    migrateKvAttachmentsFailed: '迁移中断：{message}',
    backfillImageUrls: '回填历史图片链接',
    backfillImageUrlsDesc: '将邮件正文中残留的 cid: 引用与旧 R2 绝对 URL 统一改写为 {{domain}}key 占位符，使前端能正常加载。',
    backfillImageUrlsConfirm: '确认开始回填历史邮件中的图片链接吗？建议先执行一次扫描（dry-run），确认无误后再执行写入。',
    startBackfill: '开始回填',
    backfillImageUrlsScanning: '扫描中：已扫描 {count}',
    backfillImageUrlsWriting: '写入中：已更新 {count}',
    backfillImageUrlsSuccess: '回填完成：扫描 {scanned}、已改写 {rewritten}、跳过 {skipped}、失败 {failed}',
    backfillImageUrlsFailed: '回填中断：{message}',
    backfillImageUrlsFailedRefs: '回填中断：{message}（行 {emailId}，未解析的 cid：{refs}）',
    ...
    emailText: '邮件文本',
    emailPrefix: '邮箱前缀',
    managedDomainsTitle: '邮件域名与管理员',
    managedDomains: '允许的邮箱域名',
    managedDomainsDesc: '用户注册、登录、添加邮箱时,邮箱后缀必须在列表中',
    adminEmails: '超级管理员邮箱',
    adminEmailsDesc: '拥有全部权限的管理员邮箱,支持添加多个;删除某项即可撤销该邮箱的管理员身份',
    atLeast: '至少',
    character: '位',
    mustNotContain: '禁止包含',
    ...
    sync: '同步',
    syncing: '同步中...',
    syncConfirm: '确认从邮件服务商拉取邮件及最新状态并同步到系统中？',
    syncSuccess: '同步完成，新增 {inserted} 封，更新 {updated} 封，跳过 {skipped} 封',
    syncFailed: '同步失败',
